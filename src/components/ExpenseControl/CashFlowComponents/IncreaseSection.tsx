...
import type { FinancialIncrease, FinancialIncreaseType } from '../../../types/financial'

import { FontAwesomeIcon } from '@fortawesome/react-fontawesome'
import { faPlus, faTrash, faArrowTrendUp, faChevronDown, faChevronRight } from '@fortawesome/free-solid-svg-icons'

import AddIncreaseTypeModal from './AddIncreaseTypeModal'
import AddIncreaseItemModal from './AddIncreaseItemModal'
...
type Props = {
	types: FinancialIncreaseType[]
	items: FinancialIncrease[]
...
export default function IncreaseSection({
	...
	const startEdit = (item: FinancialIncrease) => {
			...
			<div className='mb-4 flex flex-col gap-2 border-b border-gray-200 pb-3 sm:flex-row sm:items-center sm:justify-between'>
				<div className='flex flex-col gap-1 sm:flex-row sm:items-center sm:gap-3'>
					<h2 className='flex items-center gap-2 text-lg font-semibold'>
						<FontAwesomeIcon icon={faArrowTrendUp} className='text-mainColor text-xl' />
						Przychody
					</h2>

					<span className='font-semibold text-mainColor text-lg sm:text-base'>
				...
				<button
					...
					className='self-start sm:self-auto flex h-9 w-9 items-center justify-center rounded-full bg-mainColor text-white shadow-sm border border-gray-200 cursor-pointer'>
							...
							{isExpanded(type.id) && (
								<div className='relative ml-6 border-l border-gray-200'>
									{typeItems.map(item => (
										<div
											...
											<span className='absolute left-[-9px] top-4 sm:top-1/2 sm:-translate-y-1/2 flex h-4 w-4 items-center justify-center rounded-full bg-white border border-gray-200'>
												<span className='h-2 w-2 rounded-full bg-mainColor' />
											</span>
									...
									<button
										...
										className='ml-3 mt-2 mb-3 text-sm text-mainColor hover:underline cursor-pointer'>
										+ Dodaj pozycję
									</button>
								</div>
							)}
